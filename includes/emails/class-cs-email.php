...
if ( class_exists( 'CS_Email', false ) ) {
	return;
}
 ...
 * Email Class
 *
 * CommunityService Email Class which is extended by specific email template classes to add emails to CommunityService
 *
 * @class       CS_Email
 * @version     2.5.0
 ...
 */
class CS_Email extends CS_Settings_API {
	...
	/**
	 * Object this email is for, for example a student, task, or email.
	 *
	 * @var object|bool
	 */
	public $object;
	...
	/**
	 * True when the email notification is sent to students.
	 *
	 * @var bool
	 */
	 ...
	 *
	 * @deprecated 1.0 in favour of placeholders
	 * @var array
	 */
	public $find = array();
	...
	public function __construct() {
		...
		if ( is_null( $this->template_base ) ) {
			$this->template_base = CS()->plugin_path() . '/templates/';
		}
		...
		add_action( 'communityservice_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );
	}
	...
	public function format_string( $string ) {
		...
		if ( has_filter( 'communityservice_email_format_string_replace' ) || has_filter( 'communityservice_email_format_string_find' ) ) {
			...
			$string = str_replace( apply_filters( 'communityservice_email_format_string_find', $legacy_find, $this ), apply_filters( 'communityservice_email_format_string_replace', $legacy_replace, $this ), $string );
		}
		 ...
		 * @since 1.0
		 */
		return apply_filters( 'communityservice_email_format_string', str_replace( $find, $replace, $string ), $this );
	}

	/**
	 * Set the locale to the store locale for student emails to make sure emails are in the store language.
	 */
	public function setup_locale() {
		if ( $this->is_student_email() && apply_filters( 'communityservice_email_setup_locale', true ) ) {
	...
	public function restore_locale() {
		if ( $this->is_student_email() && apply_filters( 'communityservice_email_restore_locale', true ) ) {
			cs_restore_locale();
		}
	}
	...
	public function get_subject() {
		return apply_filters( 'communityservice_email_subject_' . $this->id, $this->format_string( $this->get_option( 'subject', $this->get_default_subject() ) ), $this->object );
	}
	...
	public function get_heading() {
		return apply_filters( 'communityservice_email_heading_' . $this->id, $this->format_string( $this->get_option( 'heading', $this->get_default_heading() ) ), $this->object );
	}
	...
	public function get_recipient() {
		$recipient  = apply_filters( 'communityservice_email_recipient_' . $this->id, $this->recipient, $this->object );
	...
	public function get_headers() {
		...
		if ( 'new_activity' === $this->id && $this->object  ) {
			$student_id = $this->object->get_student_id();
			$student = get_user_by('id',$student_id);
			$header .= 'Reply-to: ' . $student->display_name . ' <' . $student->user_email . ">\r\n";
		}elseif ( $this->get_from_address() && $this->get_from_name() ) {
			$header .= 'Reply-to: ' . $this->get_from_name() . ' <' . $this->get_from_address() . ">\r\n";
		}

		return apply_filters( 'communityservice_email_headers', $header, $this->id, $this->object );
	}
	...
	public function get_attachments() {
		return apply_filters( 'communityservice_email_attachments', array(), $this->id, $this->object );
	}
	...
	public function get_title() {
		return apply_filters( 'communityservice_email_title', $this->title, $this );
	}
	...
	public function get_description() {
		return apply_filters( 'communityservice_email_description', $this->description, $this );
	}
	...
	public function get_option( $key, $empty_value = null ) {
		...
		return apply_filters( 'communityservice_email_get_option', $value, $this, $value, $key, $empty_value );
	}
	...
	public function is_enabled() {
		return apply_filters( 'communityservice_email_enabled_' . $this->id, 'yes' === $this->enabled, $this->object );
	}
	...
	/**
	 * Checks if this email is student focussed.
	 *
	 * @return bool
	 */
	...
	public function style_inline( $content ) {
		if ( in_array( $this->get_content_type(), array( 'text/html', 'multipart/alternative' ), true ) ) {
			ob_start();
			cs_get_template( 'emails/email-styles.php' );
			$css = apply_filters( 'communityservice_email_styles', ob_get_clean() );
	...
	public function get_from_name() {
		$from_name = apply_filters( 'communityservice_email_from_name', get_option( 'blogname' ), $this );
	...
	public function get_from_address() {
		$from_address = apply_filters( 'communityservice_email_from_address', get_option( 'admin_email' ), $this );
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		...
		$message = apply_filters( 'communityservice_mail_content', $this->style_inline( $message ) );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'subject'    => array(
				...
				'description' => sprintf( __( 'Available placeholders: %s', 'communityservice' ), '<code>' . implode( '</code>, <code>', array_keys( $this->placeholders ) ) . '</code>' ),
	...
	public function get_email_type_options() {
		$types = array( 'plain' => __( 'Plain text', 'communityservice' ) );

		if ( class_exists( 'DOMDocument' ) ) {
			$types['html']      = __( 'HTML', 'communityservice' );
			$types['multipart'] = __( 'Multipart', 'communityservice' );
		}
	...
	protected function save_template( $template_code, $template_path ) {
		if ( current_user_can( 'edit_themes' ) && ! empty( $template_code ) && ! empty( $template_path ) ) {
			$saved = false;
			$file  = get_stylesheet_directory() . '/' . CS()->template_path() . $template_path;
			...
			if ( ! $saved ) {
				$redirect = add_query_arg( 'cs_error', urlencode( __( 'Could not write to template file.', 'communityservice' ) ) );
	...
	public function get_theme_template_file( $template ) {
		return get_stylesheet_directory() . '/' . apply_filters( 'communityservice_template_directory', 'communityservice', $template ) . '/' . $template;
	}
	...
	protected function move_template_action( $template_type ) {
		...
		if ( ! empty( $template ) ) {
			...
			if ( wp_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
				...
				$template_file = apply_filters( 'communityservice_locate_core_template', $core_file, $template, $this->template_base, $this->id );
				 ...
				 */
				do_action( 'communityservice_copy_email_template', $template_type, $this );

				?>
				<div class="updated">
					<p><?php echo esc_html__( 'Template file copied to theme.', 'communityservice' ); ?></p>
				</div>
				<?php
			}
		}
		...
		if ( $template ) {
			if ( ! empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					 ...
					 * @param string $email The email object
					 */
					do_action( 'communityservice_delete_email_template', $template_type, $this );
					?>
					<div class="updated">
						<p><?php echo esc_html__( 'Template file deleted from theme.', 'communityservice' ); ?></p>
					</div>
					<?php
				}
			}
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( empty( $_GET['_cs_email_nonce'] ) || ! wp_verify_nonce( cs_clean( wp_unslash( $_GET['_cs_email_nonce'] ) ), 'communityservice_email_template_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'communityservice' ) );
			}

			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( esc_html__( 'You don&#8217;t have permission to do this.', 'communityservice' ) );
			}

			if ( ! empty( $_GET['move_template'] ) ) {
				$this->move_template_action( cs_clean( wp_unslash( $_GET['move_template'] ) ) );
			}

			if ( ! empty( $_GET['delete_template'] ) ) {
				$this->delete_template_action( cs_clean( wp_unslash( $_GET['delete_template'] ) ) );
			}
		}
	}
	...
	public function admin_options() {
		...
		$this->admin_actions();
		?>
		<h2><?php echo esc_html( $this->get_title() ); ?> <?php cs_back_link( __( 'Return to emails', 'communityservice' ), admin_url( 'admin.php?page=cs-settings&tab=email' ) ); ?></h2>
		 ...
		 * @param string $email The email object
		 */
		do_action( 'communityservice_email_settings_before', $this );
		?>
		 ...
		 * @param string $email The email object
		 */
		do_action( 'communityservice_email_settings_after', $this );
		?>
		...
		if ( current_user_can( 'edit_themes' ) && ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) ) {
			...
			<div id="template">
				<?php
				$templates = array(
					'template_html'  => __( 'HTML template', 'communityservice' ),
					'template_plain' => __( 'Plain text template', 'communityservice' ),
				);
				...
				foreach ( $templates as $template_type => $title ) :
					...
					$core_file     = $this->template_base . $template;
					$template_file = apply_filters( 'communityservice_locate_core_template', $core_file, $template, $this->template_base, $this->id );
					$template_dir  = apply_filters( 'communityservice_template_directory', 'communityservice', $template );
					?>
					<div class="template <?php echo esc_attr( $template_type ); ?>">
						...
						<?php if ( file_exists( $local_file ) ) : ?>
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable ?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template_type ) ), 'communityservice_email_template_nonce', '_cs_email_nonce' ) ); ?>" class="delete_template button">
										<?php esc_html_e( 'Delete template file', 'communityservice' ); ?>
									</a>
								<?php endif; ?>

								<?php
								/* translators: %s: Path to template file */
								printf( esc_html__( 'This template has been overridden by your theme and can be found in: %s.', 'communityservice' ), '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php elseif ( file_exists( $template_file ) ) : ?>
							<p>
								...
								if ( is_writable( $target_dir ) ) : // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
									?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template_type ) ), 'communityservice_email_template_nonce', '_cs_email_nonce' ) ); ?>" class="button">
										<?php esc_html_e( 'Copy file to theme', 'communityservice' ); ?>
									</a>
								<?php endif; ?>

								<?php
								/* translators: 1: Path to template file 2: Path to theme folder */
								printf( esc_html__( 'To override and edit this email template copy %1$s to your theme folder: %2$s.', 'communityservice' ), '<code>' . esc_html( plugin_basename( $template_file ) ) . '</code>', '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php else : ?>
							<p><?php esc_html_e( 'File was not found.', 'communityservice' ); ?></p>
						<?php endif; ?>
					</div>
				<?php endforeach; ?>
			</div>
			...
			cs_enqueue_js(
				...
				var view = '" . esc_js( __( 'View template', 'communityservice' ) ) . "';
				var hide = '" . esc_js( __( 'Hide template', 'communityservice' ) ) . "';
				...
				jQuery( 'a.delete_template' ).click( function() {
					if ( window.confirm('" . esc_js( __( 'Are you sure you want to delete this template file?', 'communityservice' ) ) . "') ) {
						return true;
					}
