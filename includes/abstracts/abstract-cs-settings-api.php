...
/**
 * CS_Settings_API class.
 */
abstract class CS_Settings_API {
	 ...
	 * @var string
	 */
	public $plugin_id = 'communityservice_';
	...
	public function get_form_fields() {
		return apply_filters( 'communityservice_settings_api_form_fields_' . $this->id, array_map( array( $this, 'set_defaults' ), $this->form_fields ) );
	}
	...
	public function update_option( $key, $value = '' ) {
		...
		return update_option( $this->get_option_key(), apply_filters( 'communityservice_settings_api_sanitized_fields_' . $this->id, $this->settings ), 'yes' );
	}
	...
	public function get_tooltip_html( $data ) {
		...
		return $tip ? cs_help_tip( $tip, true ) : '';
	}
	...
	public function generate_price_html( $key, $data ) {
		...
		<tr valign="top">
			...
			<td class="forminp">
				<fieldset>
					<legend class="screen-reader-text"><span><?php echo wp_kses_post( $data['title'] ); ?></span></legend>
					<input class="cs_input_price input-text regular-input <?php echo esc_attr( $data['class'] ); ?>" type="text" name="<?php echo esc_attr( $field_key ); ?>" id="<?php echo esc_attr( $field_key ); ?>" style="<?php echo esc_attr( $data['css'] ); ?>" value="<?php echo esc_attr( cs_format_localized_price( $this->get_option( $key ) ) ); ?>" placeholder="<?php echo esc_attr( $data['placeholder'] ); ?>" <?php disabled( $data['disabled'], true ); ?> <?php echo $this->get_custom_attribute_html( $data ); // WPCS: XSS ok. ?> />
					<?php echo $this->get_description_html( $data ); // WPCS: XSS ok. ?>
				</fieldset>
			</td>
		</tr>
	...
	public function generate_decimal_html( $key, $data ) {
		...
		<tr valign="top">
			...
			<td class="forminp">
				<fieldset>
					<legend class="screen-reader-text"><span><?php echo wp_kses_post( $data['title'] ); ?></span></legend>
					<input class="cs_input_decimal input-text regular-input <?php echo esc_attr( $data['class'] ); ?>" type="text" name="<?php echo esc_attr( $field_key ); ?>" id="<?php echo esc_attr( $field_key ); ?>" style="<?php echo esc_attr( $data['css'] ); ?>" value="<?php echo esc_attr( cs_format_localized_decimal( $this->get_option( $key ) ) ); ?>" placeholder="<?php echo esc_attr( $data['placeholder'] ); ?>" <?php disabled( $data['disabled'], true ); ?> <?php echo $this->get_custom_attribute_html( $data ); // WPCS: XSS ok. ?> />
					<?php echo $this->get_description_html( $data ); // WPCS: XSS ok. ?>
				</fieldset>
			</td>
		</tr>
			...
			<td class="forminp">
				<fieldset>
					...
					<?php if ( $data['select_buttons'] ) : ?>
						<br/><a class="select_all button" href="#"><?php esc_html_e( 'Select all', 'communityservice' ); ?></a> <a class="select_none button" href="#"><?php esc_html_e( 'Select none', 'communityservice' ); ?></a>
					<?php endif; ?>
				</fieldset>
			</td>
		</tr>
	...
	public function generate_title_html( $key, $data ) {
		...
		?>
			...
			<h3 class="cs-settings-sub-title <?php echo esc_attr( $data['class'] ); ?>" id="<?php echo esc_attr( $field_key ); ?>"><?php echo wp_kses_post( $data['title'] ); ?></h3>
	...
	public function validate_price_field( $key, $value ) {
		$value = is_null( $value ) ? '' : $value;
		return ( '' === $value ) ? '' : cs_format_decimal( trim( stripslashes( $value ) ) );
	}
	...
	public function validate_select_field( $key, $value ) {
		$value = is_null( $value ) ? '' : $value;
		return cs_clean( stripslashes( $value ) );
	}
	...
	public function validate_multiselect_field( $key, $value ) {
		return is_array( $value ) ? array_map( 'cs_clean', array_map( 'stripslashes', $value ) ) : '';
	}
	...
	public function validate_settings_fields( $form_fields = array() ) {
		cs_deprecated_function( 'validate_settings_fields', '2.6' );
	}
	...
	public function format_settings( $value ) {
		cs_deprecated_function( 'format_settings', '2.6' );
		return $value;
	}
}
