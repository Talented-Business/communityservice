<?php
/**
 * Contains the query functions for CommunityService which alter the front-end post queries and loops
 *
 * @version 1.0
...
class CS_Query {
	...
	/**
	 * Reference to the main task query on the page.
	 *
	 * @var array
	 */
	private static $task_query;
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			...
			add_filter( 'the_posts', array( $this, 'remove_task_query_filters' ) );
	...
	public function init_query_vars() {
		...
		$this->query_vars = array(
			...
			'view-order'                 => get_option( 'communityservice_myaccount_view_order_endpoint', 'view-order' ),
			'downloads'                  => get_option( 'communityservice_myaccount_downloads_endpoint', 'downloads' ),
			'set-default-payment-method' => get_option( 'communityservice_myaccount_set_default_payment_method_endpoint', 'set-default-payment-method' ),
		);
	}
	...
	public function get_endpoint_title( $endpoint ) {
		...
		switch ( $endpoint ) {
			...
			case 'add-payment-method':
				...
				break;
			case 'lost-password':
				$title = __( 'Lost password', 'communityservice' );
				break;
			default:
				$title = '';
				break;
		...
		return apply_filters( 'communityservice_endpoint_' . $endpoint . '_title', $title, $endpoint );
	}
	...
	public function get_endpoints_mask() {
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			$myaccount_page_id = get_option( 'communityservice_myaccount_page_id' );
			$checkout_page_id  = get_option( 'communityservice_checkout_page_id' );
	...
	public function get_query_vars() {
		return apply_filters( 'communityservice_get_query_vars', $this->query_vars );
	}
	...
	public function parse_request() {
		...
		if(isset($wp->query_vars['post_type'])&&$wp->query_vars['post_type'] == 'cs-activity'){
			if(isset($wp->query_vars['author'])&&$wp->query_vars['author']>0){
				$user_ids = array($wp->query_vars['author']);
			}
			if(isset($_GET['_year'])&&$_GET['_year']>0){
				$year = intval($_GET['_year']);
				$user_ids_year = get_objects_in_term( $year, 'user-group' );
				if(isset($user_ids)&&count($user_ids)>0){
					$user_ids = array_intersect($user_ids,$user_ids_year);
				}else{
					$user_ids = $user_ids_year;
				}
			}
			if(isset($_GET['_house'])&&$_GET['_house']>0){
				$house = intval($_GET['_house']);
				$user_ids_house = get_objects_in_term( $house, 'user-type' );
				if(isset($user_ids)&&count($user_ids)>0){
					$user_ids = array_intersect($user_ids,$user_ids_house);
				}else{
					$user_ids = $user_ids_house;
				}
			}
			if(isset($user_ids)){
				if(count($user_ids)==0)$wp->query_vars['author'] = 1000000;
				else $wp->query_vars['author'] = '['.implode(',',$user_ids).']';
			}
		}
	}
	...
	/**
	 * Hook into pre_get_posts to do the main task query.
	 *
	 * @param WP_Query $q Query instance.
	 */
	public function pre_get_posts( $q ) {
		...
		// Fix task feeds.
		if ( $q->is_feed() && $q->is_post_type_archive( 'task' ) ) {
	 ...
	 * @return array
	 */
	public function remove_task_query_filters( $posts ) {
	...
	/**
	 * Query the tasks, applying sorting/ordering etc.
	 * This applies to the main WordPress loop.
	 *
	 * @param WP_Query $q Query instance.
	 */
	public function task_query( $q ) {
		if ( ! is_feed() ) {
		...
		$q->set( 'cs_query', 'task_query' );
		$q->set( 'post__in', array_unique( (array) apply_filters( 'loop_shop_post_in', array() ) ) );

		// Work out how many tasks to query.
		$q->set( 'posts_per_page', $q->get( 'posts_per_page' ) ? $q->get( 'posts_per_page' ) : apply_filters( 'loop_shop_per_page', cs_get_default_task_rows_per_page() * cs_get_default_task_rows_per_page() ) );

		// Store reference to this query.
		self::$task_query = $q;

		do_action( 'communityservice_task_query', $q, $this );
	}

	/**
	 * Remove the query.
	 */
	public function remove_task_query() {
	...
	/**
	 * Returns an array of arguments for ordering tasks based on the selected values.
	 *
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			$orderby_value = isset( $_GET['orderby'] ) ? cs_clean( (string) wp_unslash( $_GET['orderby'] ) ) : cs_clean( get_query_var( 'orderby' ) ); // WPCS: sanitization ok, input var ok, CSRF ok.

			if ( ! $orderby_value ) {
				if ( is_search() ) {
					$orderby_value = 'relevance';
				} else {
					$orderby_value = apply_filters( 'communityservice_default_catalog_orderby', get_option( 'communityservice_default_catalog_orderby', 'menu_order' ) );
				}
			}
		...
		return apply_filters( 'communityservice_get_catalog_ordering_args', $args );
	}
	...
	public function order_by_price_asc_post_clauses( $args ) {
		...
		if ( isset( $wp_query->queried_object, $wp_query->queried_object->term_taxonomy_id, $wp_query->queried_object->taxonomy ) && is_a( $wp_query->queried_object, 'WP_Term' ) ) {
			...
			$args['join']         .= " INNER JOIN (
				...
				) as tasks_within_terms ON $wpdb->postmeta.post_id = tasks_within_terms.object_id
	...
	public function get_meta_query( $meta_query = array(), $main_query = false ) {
		...
		if($user_id&&is_student()&&true){
			$year = get_year_id_user($user_id);
			$meta_query[] = array(
				'key'     => 'years',
				'value'   => 'i:'.$year.';',
				'compare' => 'REGEXP',
			);
			/*$house = get_house_id_user($user_id);
			$meta_query[]= array(
				'key'     => 'houses',
				'value'   => 'i:'.$house.';',
				'compare' => 'REGEXP',
		...
		return array_filter( apply_filters( 'communityservice_task_query_meta_query', $meta_query, $this ) );
	}
	...
	public function get_tax_query( $tax_query = array(), $main_query = false ) {
		...
		$task_visibility_terms  = array();//cs_get_task_visibility_term_ids();
		$task_visibility_not_in = array( is_search() && $main_query ? $task_visibility_terms['exclude-from-search'] : $task_visibility_terms['exclude-from-catalog'] );

		// Hide out of stock tasks.
		if ( 'yes' === get_option( 'communityservice_hide_out_of_stock_items' ) ) {
			$task_visibility_not_in[] = $task_visibility_terms['outofstock'];
		}
		...
		if ( isset( $_GET['rating_filter'] ) ) { // WPCS: input var ok, CSRF ok.
			...
			$rating_terms  = array();
			for ( $i = 1; $i <= 5; $i ++ ) {
				if ( in_array( $i, $rating_filter, true ) && isset( $task_visibility_terms[ 'rated-' . $i ] ) ) {
					$rating_terms[] = $task_visibility_terms[ 'rated-' . $i ];
				}
			}
		...
		if ( ! empty( $task_visibility_not_in ) ) {
			$tax_query[] = array(
				'taxonomy' => 'task_visibility',
				'field'    => 'term_taxonomy_id',
				'terms'    => $task_visibility_not_in,
				'operator' => 'NOT IN',
			);
		}

		return array_filter( apply_filters( 'communityservice_task_query_tax_query', $tax_query, $this ) );
	}
	...
	/**
	 * Get the main query which task queries ran against.
	 *
	 * @return array
	 */
	...
	public static function get_main_tax_query() {
		$tax_query = isset( self::$task_query->tax_query, self::$task_query->tax_query->queries ) ? self::$task_query->tax_query->queries : array();
	...
	public static function get_main_meta_query() {
		$args       = self::$task_query->query_vars;
			...
			if ( ! empty( $_GET ) ) { // WPCS: input var ok, CSRF ok.
				foreach ( $_GET as $key => $value ) { // WPCS: input var ok, CSRF ok.
					if ( 0 === strpos( $key, 'filter_' ) ) {
						...
						$filter_terms = ! empty( $value ) ? explode( ',', cs_clean( wp_unslash( $value ) ) ) : array();

						if ( empty( $filter_terms ) || ! taxonomy_exists( $taxonomy ) || ! wc_attribute_taxonomy_id_by_name( $attribute ) ) {
							continue;
						}

						$query_type                                     = ! empty( $_GET[ 'query_type_' . $attribute ] ) && in_array( $_GET[ 'query_type_' . $attribute ], array( 'and', 'or' ), true ) ? cs_clean( wp_unslash( $_GET[ 'query_type_' . $attribute ] ) ) : ''; // WPCS: sanitization ok, input var ok, CSRF ok.
						self::$_chosen_attributes[ $taxonomy ]['terms'] = array_map( 'sanitize_title', $filter_terms ); // Ensures correct encoding.
						self::$_chosen_attributes[ $taxonomy ]['query_type'] = $query_type ? $query_type : apply_filters( 'communityservice_layered_nav_default_query_type', 'and' );
					}
				}
			}
		}
	...
	/**
	 * Returns a meta query to handle task visibility.
	 *
	 * @deprecated 3.0.0 Replaced with taxonomy.
	...
	/**
	 * Returns a meta query to handle task stock status.
	 *
	 * @deprecated 3.0.0 Replaced with taxonomy.
	...
	/**
	 * Get an unpaginated list all task IDs (both filtered and unfiltered). Makes use of transients.
	 *
	 * @deprecated 2.6.0 due to performance concerns
	 */
	public function get_tasks_in_view() {
		wc_deprecated_function( 'get_tasks_in_view', '2.6' );
	}
	...
	public function search_post_excerpt( $where = '' ) {
		wc_deprecated_function( 'CS_Query::search_post_excerpt', '3.2.0', 'Excerpt added to search query by default since WordPress 4.5.' );
		return $where;
	}
	...
	public function remove_posts_where() {
		wc_deprecated_function( 'CS_Query::remove_posts_where', '3.2.0', 'Nothing to remove anymore because search_post_excerpt() is deprecated.' );
	}
