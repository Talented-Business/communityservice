...
class CS_Cache_Helper {
	...
	public static function get_cache_prefix( $group ) {
		// Get cache key - uses cache key cs_orders_cache_prefix to invalidate when needed.
		$prefix = wp_cache_get( 'cs_' . $group . '_cache_prefix', $group );

		if ( false === $prefix ) {
			$prefix = 1;
			wp_cache_set( 'cs_' . $group . '_cache_prefix', $prefix, $group );
		}

		return 'cs_cache_' . $prefix . '_';
	}
	...
	public static function incr_cache_prefix( $group ) {
		wp_cache_incr( 'cs_' . $group . '_cache_prefix', 1, $group );
	}
	...
	public static function geolocation_ajax_get_location_hash() {
		$customer             = new CS_Customer( 0, true );
		$location             = array();
	...
	public static function prevent_caching() {
		...
		$page_ids = array_filter( array( cs_get_page_id( 'cart' ), cs_get_page_id( 'checkout' ), cs_get_page_id( 'myaccount' ) ) );
	...
	public static function geolocation_ajax_redirect() {
		if ( 'geolocation_ajax' === get_option( 'woocommerce_default_customer_address' ) && ! is_checkout() && ! is_cart() && ! is_account_page() && ! is_ajax() && empty( $_POST ) ) { // WPCS: CSRF ok, input var ok.
			...
			$current_hash  = isset( $_GET['v'] ) ? cs_clean( wp_unslash( $_GET['v'] ) ) : ''; // WPCS: sanitization ok, input var ok, CSRF ok.
	...
	public static function set_nocache_constants( $return = true ) {
		cs_maybe_define_constant( 'DONOTCACHEPAGE', true );
		cs_maybe_define_constant( 'DONOTCACHEOBJECT', true );
		cs_maybe_define_constant( 'DONOTCACHEDB', true );
		return $return;
	}
	...
	public static function notices() {
		...
		if ( $enabled && ! in_array( '_cs_session_', $settings, true ) ) {
			?>
			<div class="error">
				<p>
				<?php
				/* translators: 1: key 2: URL */
				echo wp_kses_post( sprintf( __( 'In order for <strong>database caching</strong> to work with CommunityService you must add %1$s to the "Ignored Query Strings" option in <a href="%2$s">W3 Total Cache settings</a>.', 'woocommerce' ), '<code>_cs_session_</code>', esc_url( admin_url( 'admin.php?page=w3tc_dbcache' ) ) ) );
				?>
				</p>
			</div>
			<?php
	...
	/**
	 * Clean term caches added by CommunityService.
	 *
	 * @since 3.3.4
