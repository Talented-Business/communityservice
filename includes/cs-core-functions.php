...
require CS_ABSPATH . 'includes/cs-conditional-functions.php';
require CS_ABSPATH . 'includes/cs-user-functions.php';
require CS_ABSPATH . 'includes/cs-task-functions.php';
require CS_ABSPATH . 'includes/cs-formatting-functions.php';
require CS_ABSPATH . 'includes/cs-page-functions.php';
require CS_ABSPATH . 'includes/cs-activity-functions.php';
require CS_ABSPATH . 'includes/cs-account-functions.php';
/*require CS_ABSPATH . 'includes/wc-user-functions.php';
require CS_ABSPATH . 'includes/wc-order-item-functions.php';
require CS_ABSPATH . 'includes/wc-stock-functions.php';
require CS_ABSPATH . 'includes/wc-term-functions.php';
require CS_ABSPATH . 'includes/wc-attribute-functions.php';
require CS_ABSPATH . 'includes/wc-rest-functions.php';
require CS_ABSPATH . 'includes/wc-widget-functions.php';
require CS_ABSPATH . 'includes/wc-webhook-functions.php';*/
...
/**
 * Create a new activity programmatically.
 *
 * Returns a new activity object on success which can then be used to add additional data.
 *
 * @param  array $args Activity arguments.
 * @return CS_Activity|WP_Error
 */
function cs_create_activity( $args = array() ) {
	$default_args = array(
		'status'        => null,
		'student_id'   => null,
		'student_note' => null,
	...
	try {
		...
		$activity = new CS_Activity( $args['activity_id'] );

		// Update props that were set (not null).
		if ( ! is_null( $args['parent'] ) ) {
			$activity->set_parent_id( absint( $args['parent'] ) );
		}

		if ( ! is_null( $args['status'] ) ) {
			$activity->set_status( $args['status'] );
		}

		if ( ! is_null( $args['student_note'] ) ) {
			$activity->set_student_note( $args['student_note'] );
		}

		if ( ! is_null( $args['student_id'] ) ) {
			$activity->set_student_id( is_numeric( $args['student_id'] ) ? absint( $args['student_id'] ) : 0 );
		}

		if ( ! is_null( $args['created_via'] ) ) {
			$activity->set_created_via( sanitize_text_field( $args['created_via'] ) );
		}

		if ( ! is_null( $args['cart_hash'] ) ) {
			$activity->set_cart_hash( sanitize_text_field( $args['cart_hash'] ) );
		}

		// Set these fields when creating a new activity but not when updating an existing activity.
		if ( ! $args['activity_id'] ) {
			$activity->set_currency( get_woocommerce_currency() );
			$activity->set_prices_include_tax( 'yes' === get_option( 'woocommerce_prices_include_tax' ) );
			$activity->set_student_ip_address( CS_Geolocation::get_ip_address() );
			$activity->set_student_user_agent( cs_get_user_agent() );
		}

		// Update other activity props set automatically.
		$activity->save();
	} catch ( Exception $e ) {
...
/**
 * Update an activity. Uses cs_create_activity.
 *
 * @param  array $args Activity arguments.
 * @return CS_Activity|WP_Error
 */
function cs_update_activity( $args ) {
	if ( empty( $args['activity_id'] ) ) {
		return new WP_Error( __( 'Invalid activity ID.', 'woocommerce' ) );
	}
	return cs_create_activity( $args );
}

/**
 * Get template part (for templates like the shop-loop).
 *
 * CS_TEMPLATE_DEBUG_MODE will prevent overrides in themes from taking priority.
 *
 * @param mixed  $slug Template slug.
 * @param string $name Template name (default: '').
 */
function cs_get_template_part( $slug, $name = '' ) {
	$template = '';

	// Look in yourtheme/slug-name.php and yourtheme/woocommerce/slug-name.php.
	if ( $name && ! CS_TEMPLATE_DEBUG_MODE ) {
		$template = locate_template( array( "{$slug}-{$name}.php", CS()->template_path() . "{$slug}-{$name}.php" ) );
	}

	// Get default slug-name.php.
	if ( ! $template && $name && file_exists( CS()->plugin_path() . "/templates/{$slug}-{$name}.php" ) ) {
		$template = CS()->plugin_path() . "/templates/{$slug}-{$name}.php";
	}

	// If template file doesn't exist, look in yourtheme/slug.php and yourtheme/woocommerce/slug.php.
	if ( ! $template && ! CS_TEMPLATE_DEBUG_MODE ) {
		$template = locate_template( array( "{$slug}.php", CS()->template_path() . "{$slug}.php" ) );
	}

	// Allow 3rd party plugins to filter template file from their plugin.
	$template = apply_filters( 'cs_get_template_part', $template, $slug, $name );
 ...
 */
function cs_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	if ( ! empty( $args ) && is_array( $args ) ) {
		extract( $args ); // @codingStandardsIgnoreLine
	}

	$located = cs_locate_template( $template_name, $template_path, $default_path );

	if ( ! file_exists( $located ) ) {
		/* translators: %s template */
		//cs_doing_it_wrong( __FUNCTION__, sprintf( __( '%s does not exist.', 'woocommerce' ), '<code>' . $located . '</code>' ), '2.1' );
		return;
	}

	// Allow 3rd party plugin filter template file from their plugin.
	$located = apply_filters( 'cs_get_template', $located, $template_name, $args, $template_path, $default_path );
...
/**
 * Like cs_get_template, but returns the HTML instead of outputting.
 *
 * @see cs_get_template
 * @since 2.5.0
 * @param string $template_name Template name.
 ...
 * @return string
 */
function cs_get_template_html( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
	ob_start();
	cs_get_template( $template_name, $args, $template_path, $default_path );
	return ob_get_clean();
}
/**
 * Locate a template and return the path for inclusion.
 *
 * This is the load activity:
 *
 ...
 * @return string
 */
function cs_locate_template( $template_name, $template_path = '', $default_path = '' ) {
	if ( ! $template_path ) {
		$template_path = CS()->template_path();
	}

	if ( ! $default_path ) {
		$default_path = CS()->plugin_path() . '/templates/';
	}
 ...
 */
function cs_mail( $to, $subject, $message, $headers = "Content-Type: text/html\r\n", $attachments = '' ) {
	$mailer = CS()->mailer();
 ...
 */
function cs_get_theme_support( $prop = '', $default = null ) {
	$theme_support = get_theme_support( 'communityservice' );
...
function wc_get_image_size( $image_size ) {
	...
	if ( is_array( $image_size ) ) {
		...
		if ( 'single' === $image_size ) {
			$size['width']  = absint( cs_get_theme_support( 'single_image_width', get_option( 'woocommerce_single_image_width', 600 ) ) );
			$size['height'] = '';
			$size['crop']   = 0;

		} elseif ( 'gallery_thumbnail' === $image_size ) {
			$size['width']  = absint( cs_get_theme_support( 'gallery_thumbnail_image_width', 100 ) );
			$size['height'] = $size['width'];
			$size['crop']   = 1;

		} elseif ( 'thumbnail' === $image_size ) {
			$size['width'] = absint( cs_get_theme_support( 'thumbnail_image_width', get_option( 'woocommerce_thumbnail_image_width', 300 ) ) );
...
function get_woocommerce_api_url( $path ) {
	$version = defined( 'CS_API_REQUEST_VERSION' ) ? CS_API_REQUEST_VERSION : substr( CS_API::VERSION, 0, 1 );
...
function wc_get_log_file_path( $handle ) {
	return CS_Log_Handler_File::get_log_file_path( $handle );
}
...
function wc_get_log_file_name( $handle ) {
	return CS_Log_Handler_File::get_log_file_name( $handle );
}
 ...
 * @return array
 */
function cs_format_country_state_string( $country_string ) {
...
/**
 * Get the student's default location.
 *
 ...
 * @return array
 */
function cs_get_student_default_location() {
	$location = array();

	switch ( get_option( 'communityservice_default_student_address' ) ) {
		case 'geolocation_ajax':
		case 'geolocation':
			// Exclude common bots from geolocation by user agent.
			$ua = strtolower( cs_get_user_agent() );

			if ( ! strstr( $ua, 'bot' ) && ! strstr( $ua, 'spider' ) && ! strstr( $ua, 'crawl' ) ) {
				$location = CS_Geolocation::geolocate_ip( '', true, false );
			}

			// Base fallback.
			if ( empty( $location['country'] ) ) {
				$location = cs_format_country_state_string( apply_filters( 'communityservice_student_default_location', get_option( 'communityservice_default_country' ) ) );
			}
			break;
		case 'base':
			$location = cs_format_country_state_string( apply_filters( 'communityservice_student_default_location', get_option( 'communityservice_default_country' ) ) );
			break;
		default:
			$countries = array();//CS()->countries->get_allowed_countries();
			$location  = cs_format_country_state_string( apply_filters( 'communityservice_student_default_location', 1 === count( $countries ) ? key( $countries ) : '' ) );
			break;
	}

	return apply_filters( 'communityservice_student_default_location_array', $location );
}
 ...
 * @return string
 */
function cs_get_user_agent() {
	return isset( $_SERVER['HTTP_USER_AGENT'] ) ? cs_clean( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) : ''; // @codingStandardsIgnoreLine
}
...
function wc_transaction_query( $type = 'start', $force = false ) {
	...
	wc_maybe_define_constant( 'CS_USE_TRANSACTIONS', true );

	if ( CS_USE_TRANSACTIONS || $force ) {
		switch ( $type ) {
			case 'commit':
				$wpdb->query( 'COMMIT' );
				break;
...
function woocommerce_register_shipping_method( $shipping_method ) {
	CS()->shipping->register_shipping_method( $shipping_method );
}
...
if ( ! function_exists( 'wc_get_shipping_zone' ) ) {
	 ...
	 *
	 * @since  2.6.0
	 * @uses   CS_Shipping_Zones::get_zone_matching_package
	 * @param  array $package Shipping package.
	 * @return CS_Shipping_Zone
	 */
	function wc_get_shipping_zone( $package ) {
		return CS_Shipping_Zones::get_zone_matching_package( $package );
	}
}
...
function wc_get_shipping_method_count( $include_legacy = false ) {
	...
	$transient_name = 'wc_shipping_method_count_' . ( $include_legacy ? 1 : 0 ) . '_' . CS_Cache_Helper::get_transient_version( 'shipping' );
	...
	if ( false === $method_count ) {
		...
		if ( $include_legacy ) {
			...
			$methods = CS()->shipping->get_shipping_methods();
 ...
 * @since 1.0
 */
function cs_nocache_headers() {
	//CS_Cache_Helper::set_nocache_constants();
	nocache_headers();
}
...
/**
 * Get rounding precision for internal CS calculations.
 * Will increase the precision of wc_get_price_decimals by 2 decimals, unless CS_ROUNDING_PRECISION is set to a higher number.
 *
 * @since 2.6.3
 * @return int
 */
function wc_get_rounding_precision() {
	...
	if ( absint( CS_ROUNDING_PRECISION ) > $precision ) {
		$precision = absint( CS_ROUNDING_PRECISION );
	}
	return $precision;
}
 ...
 *
 * Use the communityservice_logging_class filter to change the logging class. You may provide one of the following:
 *     - a class name which will be instantiated as `new $class` with no arguments
 *     - an instance which will be used directly as the logger
 * In either case, the class or instance *must* implement CS_Logger_Interface.
 *
 * @see CS_Logger_Interface
 *
 * @return CS_Logger
 */
function cs_get_logger() {
	static $logger = null;

	$class = apply_filters( 'communityservice_logging_class', 'CS_Logger' );
	...
	if ( is_array( $implements ) && in_array( 'CS_Logger_Interface', $implements, true ) ) {
		$logger = is_object( $class ) ? $class : new $class();
	} else {
		/*cs_doing_it_wrong(
			__FUNCTION__,
			sprintf(
				__( 'The class %1$s provided by %2$s filter must implement %3$s.', 'communityservice' ),
				'<code>' . esc_html( is_object( $class ) ? get_class( $class ) : $class ) . '</code>',
				'<code>communityservice_logging_class</code>',
				'<code>CS_Logger_Interface</code>'
			),
			'3.0'
		);*/

		$logger = is_a( $logger, 'CS_Logger' ) ? $logger : new CS_Logger();
	}
...
function wc_register_default_log_handler( $handlers ) {
	if ( defined( 'CS_LOG_HANDLER' ) && class_exists( CS_LOG_HANDLER ) ) {
		$handler_class   = CS_LOG_HANDLER;
		$default_handler = new $handler_class();
	} else {
		$default_handler = new CS_Log_Handler_File();
	}
 ...
 * @return array
 */
function cs_get_permalink_structure() {
	$saved_permalinks = (array) get_option( 'communityservice_permalinks', array() );
	$permalinks       = wp_parse_args(
		array_filter( $saved_permalinks ), array(
			'task_base'           => _x( 'task', 'slug', 'communityservice' ),
			'category_base'          => _x( 'task-category', 'slug', 'communityservice' ),
			'tag_base'               => _x( 'task-tag', 'slug', 'communityservice' ),
			'attribute_base'         => '',
	...
	if ( $saved_permalinks !== $permalinks ) {
		update_option( 'communityservice_permalinks', $permalinks );
	}

	$permalinks['task_rewrite_slug']   = untrailingslashit( $permalinks['task_base'] );
...
/**
 * Switch CommunityService to site language.
 *
 * @since 3.1.0
 */
function cs_switch_to_site_locale() {
...
function wc_get_post_data_by_key( $key, $default = '' ) {
	return wc_clean( wp_unslash( cs_get_var( $_POST[ $key ], $default ) ) ); // @codingStandardsIgnoreLine
}
 ...
 * @return mixed
 */
function cs_get_var( &$var, $default = null ) {
...
function wc_enable_wc_plugin_headers( $headers ) {
	if ( ! class_exists( 'CS_Plugin_Updates' ) ) {
		include_once dirname( __FILE__ ) . '/admin/plugin-updates/class-wc-plugin-updates.php';
	}

	$headers['CSRequires'] = CS_Plugin_Updates::VERSION_REQUIRED_HEADER;
	$headers['CSTested']   = CS_Plugin_Updates::VERSION_TESTED_HEADER;
	return $headers;
}
...
function wc_prevent_dangerous_auto_updates( $should_update, $plugin ) {
	...
	$plugin_updates   = new CS_Plugin_Updates();
 ...
 * @return boolean
 */
function cs_is_active_theme( $theme ) {
...
function wc_cleanup_session_data() {
	$session_class = apply_filters( 'woocommerce_session_handler', 'CS_Session_Handler' );
...
function wc_round_discount( $value, $precision ) {
	if ( version_compare( PHP_VERSION, '5.3.0', '>=' ) ) {
		return round( $value, $precision, CS_DISCOUNT_ROUNDING_MODE ); // phpcs:ignore PHPCompatibility.PHP.NewFunctionParameters.round_modeFound
	}

	if ( 2 === CS_DISCOUNT_ROUNDING_MODE ) {
